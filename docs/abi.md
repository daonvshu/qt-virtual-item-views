...
| 2 | **改**虚函数签名 / 参数默认值语义、**删除**虚函数、或**新增纯虚**函数 | 破坏，进主版本 | vtable 布局变化；删除 / 改签名还会让既有子类的 `override` 编译不过或行为错位 |
| 3 | **新增带默认实现的 non-pure 虚函数**（典型例子：给 `HeaderWidgetAdapter` 加一个可选钩子，如第三 / 第四轮加的 `setLabelModel()` / `setGeometryModel()`） | **不破坏源码**：既有子类不需要实现它，重编即可 —— 不能把它当成主版本级改动 | vtable 布局变化，所以它只能出现在"要求重编"的版本里；这正是"1.x 只承诺源码兼容"换来的空间（把这条写准，免得以后加一个可选钩子被自己的文档逼着升 2.0） |
| 4 | 改公开类**私有数据成员**的布局（增删、改类型、改对齐） | 不破坏（私有成员不是源码契约） | **允许随次版本改**：`SOVERSION` 不变，使用者升级时必须重编 —— 同上，方案 B 换来的空间 |
| 5 | 改**按值出现**的公开结构体（`TablePane`、`ColumnGeometry`、`ItemPane`、`MaterializedItem`…）的成员 | 破坏，进主版本 | 即使只加私有成员也会改 sizeof/偏移，所以不区分公私一律进主版本 |
| 6 | 改枚举的**值** | 破坏（数值 + `Q_ENUM` 元数据），进主版本 | 追加在末尾是源码兼容但仍改元数据，要在 CHANGELOG 标注 |
| 7 | 改内联函数体里"调用方会内联展开"的语义（例如某 getter 突然开始懒构造） | 语义变化，按 [api-stability.md](api-stability.md) 第 2 条的"不改默认值语义"处理 | 允许随次版本改，要求重编 |
| 8 | 改公开类的基类列表（`QWidget`/`QHeaderView` 之类） | 破坏，进主版本 | 对象布局变化 |
| 9 | 改最低 Qt 版本或构建选项组合（例如 "6.2+" 改成 "6.5+"） | 破坏，进主版本 | 依赖的符号集合变化 |

不算破坏（可以随次版本发）：新增非虚函数、新增信号、新增类、新增枚举值（追加在末尾）、
改注释与文档、改内部实现（`src/**`）、修 bug（包括上面第 3、4、7 类那种"要重编"的调整）。
