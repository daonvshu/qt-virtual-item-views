...
`restoreHeaderState()` 是**事务性**的：先把整段状态解析并校验完（含 `columnStateSize` 与
`int` 范围的检查，以及冻结集合的条数与索引范围），全部合法后才一次性提交。所以"返回 false"
就意味着视图没有被改动 —— 早先的实现是先 `restoreState(columnState)` 再解析后半段，尾部一旦
损坏就会留下"列状态已经换了、冻结设置没换"的半恢复视图。
