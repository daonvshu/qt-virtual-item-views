...
- [ ] **1c 行冻结**（§31 只定义了列方向）：**在范围内**（用户 2026-09-25 决定），规格见
      ...
      - [x] 第 1-4 步：内核行区间视图（`verticalOffset` 与最大偏移语义不变 = "冻结不产生额外滚动空间"）、
            ...
            折回。测试 `tests/unit/tst_frozenrows` 11 个用例（含 100 万行仍只物化冻结行 + 窗口）。
      - [ ] 第 5 步：纵向表头（行号条）按 pane 切分 —— 在此之前行号条仍按单一偏移绘制，是本步的
            已知中间状态。
      - [ ] 第 6 步：行冻结状态进表格级持久化（§32，版本号 +1）。
      - [ ] 第 7 步：`examples/table_frozen_rows` 与 README/roadmap 收尾。
...
| 2026-09-25 | 拖动重排的动画暂时关闭（每格提交一次的老实现先保持即时） | 那个实现每越过一个邻居就提交一次，动画只会让 section 更跟不上光标；等重做成"预览 + 松手一次提交"时，再把单次过渡接回手势上 |
