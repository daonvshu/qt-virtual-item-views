...
| **Wave 2 数据 / 状态正确性** | ✅ P1-1 列的 insert/remove/move remap（[features 见 CHANGELOG]，回归测试 `tst_headerstructure`）<br>✅ P1-2 动态高度锚点：先捕获锚点再改尺寸，锚点内容偏移基于滚动 pane（`scrollOffset + frozenTopExtent()`）；回归测试 `tst_dynamicanchor`（已验证旧实现会失败）<br>待做：P1-7 `RowSizePolicy` 与行号条一致、P1-8 rootIndex 持久化、P1-9 选择语义（Space / 多选 / 整行）、P1-10 `scrollToColumn()` 的 pane 感知、P1-11 span 重叠校验、P1-13 冻结行下的拖放坐标 | 进行中 |
