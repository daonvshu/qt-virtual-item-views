...
| **Wave 2 数据 / 状态正确性** | ✅ P1-1 列的 insert/remove/move remap（回归测试 `tst_headerstructure`）<br>✅ P1-2 动态高度锚点（回归测试 `tst_dynamicanchor`，已验证旧实现会失败）<br>✅ P1-8 rootIndex 改成持久索引 + 校验（回归测试 `tst_virtuallistview`）<br>✅ P1-9 选择语义统一到 `selectionFlagsFor()`（NoSelection 不再被 Space 选中、SelectRows 切换整行；回归测试 `tst_selection`）<br>待做：P1-7 `RowSizePolicy` 与行号条一致、P1-10 `scrollToColumn()` 的 pane 感知、P1-11 span 重叠校验、P1-13 冻结行下的拖放坐标 | 进行中 |
