...
| **Wave 2 数据 / 状态正确性** | ✅ P1-1 列的 insert/remove/move remap（`tst_headerstructure`）<br>✅ P1-2 动态高度锚点（`tst_dynamicanchor`）<br>✅ P1-8 rootIndex 持久化 + 校验（`tst_virtuallistview`）<br>✅ P1-9 选择语义统一（`tst_selection`）<br>✅ P1-10 `scrollToColumn()` 的 pane 感知（`tst_tablepanes`）<br>✅ P1-13 冻结行下的拖放坐标（`tst_dndfrozen`）<br>待做：P1-7 `RowSizePolicy` 与行号条一致、P1-11 span 重叠校验 | 进行中 |
