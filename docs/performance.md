...
* **树的 expand/collapse 是"原地 splice + memmove"（v1.0 收口时按 P2-10 改）**：`expand()` 以前
  ...
  带宽跑：要再往下只能把可见行表换成 rope / 分块 / 隐式树，工作量不小，所以它是**下一个主版本**
  的议题，1.x 期间接受这个数字（决策见 [roadmap.md](roadmap.md) §6/§7）。注：这条最早的理由是
  "改 `TreeVisibilityIndex` 的成员布局属于 ABI 破坏"，而 1.0 选的发布策略是只承诺源码兼容
  （[abi.md](abi.md) §1）—— 私有成员布局可以随次版本改，只要使用者重编；所以推迟是排期选择，
  不是兼容性约束。
